package com.invoiceapp.android.util;

/**
 * Created by RWS 6 on 1/2/2017.
 */

public class Constant {
    public static final String URL = "http://www.marvellousinstitute.com/API/register_log_API.php";

    public static final String PROGRESS = "progress";
    public static final String MESSAGE = "message";
    public static final String LIST = "list";
    public static String USER_ID = "user_id";

    public static final String PRODUCT_ID = "product_id";
    public static final String OTHER_USER_ID = "other_user_id";
    public static final String LOGIN_USER_ID = "login_user_id";
    public static final String OTHER_USER_NAME = "other_user_name";
    public static final String OFFER_PRICE = "offer_price";
    public static final String STATUS = "status";
    public static final String ADMIN_ID = "admin_id";
    public static final String PRODUCT_IMAGES = "product_images";
    public static final String PRODUCT_NAME = "product_name";
    public static final String USER_IMAGE = "user_image";

    public static final String VALID = "valid";

    public static final String PAYMENT_TYPE_ONLINE = "online";
    public static final String PAYMENT_TYPE_CASH = "cash";

    public static final String FCM_BROADCAST_KEY = "com_bikespot_android_chat";
    public static final String DATE_FORMAT = "yyyy-MM-dd HH:mm:ss";
    public static final String DEVICE_TYPE = "a";

    public static final String FROM_MAKE_OFFER = "from_make_offer";
    public static final String FROM_CHAT = "from_chat";
    public static final String FROM = "from"; // key

    /*webservice method name*/
    public static final String SUCCESS = "success";

    public static final String LOGIN_METHOD = "user_login";
    public static final String REGISTRATION_METHOD = "user_signup";
    public static final String METHOD_BUSINESS_DETAILS = "businessDetail";


    public static String PATH_PRODUCT_REPORT = "ppp";
    public static String reportName = "onetwo";
}
